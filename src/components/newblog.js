...
import {
    Box, Flex, Input, Text, Tag, Button, Center, Icon, TagRightIcon, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, useDisclosure, Accordion, AccordionItem,
    AccordionButton,
    AccordionPanel,
    AccordionIcon,
    Image,
...
const NewBlog = () => {
    ...
    return (
        supabase.auth.session() == null ?
            ...
            :
            <Center w="100%" >
                <Prompt message="Sure want to leave? Changes will not be saved." when={title == '' && body == '' && selectedImg == {}} />
                    ...
                    <Flex w={["100%", "100%", "90%", "90%"]} h="fit-content" direction="column" pb="30px" align="center" >
                        ...
                        <Box w="90%" minHeight="65vh">
                                ...
                                <AccordionItem borderY='none' width='100%'>
                                    ...
                                    <AccordionPanel pb={4} w='100%'>
                                        <form onSubmit={e => searchImage(e)}>
                                            <Input placeholder="Search image" width='fit-content' onChange={e => setImageTitle(e.target.value)} />
                                        </form>
                                        <Flex w='100%' h='fit-content' marginY='20px' justify='center'>
                                            {
                                                Object.keys(selectedImg).length == 0 &&
                                                <Masonry className="cardcon" breakpointCols={{ default: 4, 1100: 3, 700: 2, 500: 1 }}>
                                                    {
                                                        image.map(i => (
                                                            <Image key={i.id} src={i.webformatURL} onClick={e => setSelectedImg(i)} padding='10px' boxSize='auto' objectFit='cover' transitionDuration='.2s' cursor='pointer' _hover={{ transform: 'translateY(-5px)' }} />
                                                        ))
                                                    }
                                                </Masonry>
                                            }
                            ...
                            <Flex w='100%' h='fit-content' marginY='20px' justify='center'>
                                {
                                    Object.keys(selectedImg).length > 0 &&
                                    <Flex pos='relative' w='fit-content' justify='center' marginBottom='15px'>
                                        <Image key={selectedImg.id} src={selectedImg.webformatURL} />
                                        <Icon as={AiOutlineClose} onClick={() => setSelectedImg({})} pos='absolute' top='3' right='3' borderRadius='md' transitionDuration='.2s' w='fit-content' h='fit-content' cursor='pointer' padding='8px' bgColor={'grey'} opacity={.5} _hover={{ opacity: 1 }} />
                                        <Box pos='absolute' bottom='-30px' color='white'>
                                            By&nbsp;
                                            <a href={`http://pixabay.com/users/${selectedImg.user}`} style={{ textDecoration: 'underline' }} target="_blank" rel="noopener noreferrer">{selectedImg.user}</a>
                                            &nbsp;on&nbsp;
                                            <a href={selectedImg.pageURL} style={{ textDecoration: 'underline' }} target="_blank" rel="noopener noreferrer">PixaBay</a>
                                        </Box>
                                    </Flex>
                                }
                            </Flex>
